...
/**
 * Mirrors LatestFileBlobStorePanel but for readonly use with block size removed.
 * @author mike.dolding
 */
public class LatestFileBlobStorePanel extends Panel  {

    private static final long serialVersionUID = -3843873237066531610L;

    public LatestFileBlobStorePanel(String id, final IModel<LatestFileBlobStoreInfo> configModel) {
    ...
    public void onInitialize() {
        ...
        add(
                new TextField<Integer>("match")
                        .setRequired(true)
                        .add(
                                new AttributeModifier(
                                        "title", new ResourceModel("match.title"))));
        add(
                new TextField<Integer>("replace")
                        .setRequired(true)
                        .add(
                                new AttributeModifier(
                                        "title", new ResourceModel("match.title"))));
